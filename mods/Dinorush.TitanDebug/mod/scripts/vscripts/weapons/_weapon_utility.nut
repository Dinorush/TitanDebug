...
function ClusterRocket_Detonate( entity rocket, vector normal )
{
	...
	// Fix for preserving effects through Vortex. Mods are only added when setting is on.
	if ( "storedReflectMods" in rocket.s )
	{
...
function ClusterRocketBursts( vector origin, int damage, int damageHeavyArmor, float innerRadius, float outerRadius, entity owner, PopcornInfo popcornInfo, customFxTable = null )
{
	...
	AI_CreateDangerousArea_Static( clusterExplosionEnt, null, outerRadius, TitanDebug_GetSetting( "titan_debug_ai_dodge_friendly" ) ? owner.GetTeam() : TEAM_INVALID, true, true, origin )
...
void function WeaponAttackWave( entity ent, int projectileCount, entity inflictor, vector pos, vector dir, bool functionref( entity, int, entity, entity, vector, vector, int ) waveFunc )
{
	...
	for ( int i = 0; i < maxCount; i++ )
	{
		...
		// Fix for waves despawning on Particle defensives when spawned on top of them.
		if ( !TitanDebug_GetSetting( "titan_debug_wave_no_spawn_despawn" ) )
			firstTrace = false
		...
		if ( vortexHit )
		{
			...
			// Fix gas canister being entirely despawned by only blocking in the direction of the defensive
			if ( !TitanDebug_GetSetting( "titan_debug_canister_half_block" )
					|| !IsValid( projectile )
					|| projectile.ProjectileGetWeaponClassName() != "mp_titanability_slow_trap"
					|| ( traceEndOver - traceStart ).Dot( vortexHit.vortex.GetOrigin() - traceStart ) < 0 )
			{
				WaitFrame()
				continue
			}
