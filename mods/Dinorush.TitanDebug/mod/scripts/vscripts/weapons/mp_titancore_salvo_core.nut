	...
	while ( endTime > Time() )
	{
		if ( !IsValid_ThisFrame( weapon ) )
			...
			if ( SoulHasPassive( soul, ePassives.PAS_SALVO_CORE ) )
			{
				...
				bool isDoneFiring = !TitanDebug_GetSetting( "titan_debug_salvo_core_no_despawn" ) || weapon.GetBurstFireShotsPending() == weapon.GetWeaponBurstFireCount()
				if ( weapon.w.salvoMissileArray.len() == 0 && isDoneFiring )
					break
			}
		#endif
