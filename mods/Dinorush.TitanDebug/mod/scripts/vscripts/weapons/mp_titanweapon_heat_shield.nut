...
bool function OnWeaponVortexHitProjectile_titanweapon_heat_shield( entity weapon, entity vortexSphere, entity attacker, entity projectile, vector contactPos )
{
		...
		// Fixes Thermal Shield despawning projectiles and canister fire. Canister needs a special check since we want to block the canister itself but not the fire.
		// Normally handled in TryVortexAbsorb, but Thermal Shield doesn't call that.
		bool canisterFire = projectile.ProjectileGetWeaponClassName() == "mp_titanability_slow_trap" && !projectile.IsSolid()
		if ( TitanDebug_GetSetting( "titan_debug_wave_no_block_despawn" ) )
			if ( canisterFire || projectile.ProjectileGetWeaponInfoFileKeyField( "projectile_ignores_vortex" ) == "mirror" )
				return false
		else if ( TitanDebug_GetSetting( "titan_debug_canister_half_block" ) && canisterFire ) // Need to prevent canister from despawning for the half block to work right.
			return false
