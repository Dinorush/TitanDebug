...
function Vortex_Init()
{
	...
	// Need to fix vortex preserving attachments via damage callback.
	// Not an exhaustive list of classes, but hits most that matter.
	#if SERVER
	if ( TitanDebug_GetSetting( "titan_debug_vortex_carry_effects" ) )
	{
		AddDamageCallback( "player", TitanDebug_FixVortexRefire )
		AddDamageCallback( "npc_titan", TitanDebug_FixVortexRefire )
		AddDamageCallback( "npc_super_spectre", TitanDebug_FixVortexRefire )
	}
	#endif
}
...
bool function TryVortexAbsorb( entity vortexSphere, entity attacker, vector origin, int damageSourceID, entity weapon, string weaponName, string impactType, entity projectile = null, damageType = null, reflect = false )
{
	...
	if ( projectile )
	{
		...
		// Fix for wave attacks getting despawned by vortex. Thermal Shield handles this in its own file.
		// Canister wave attacks use "fall_vortex" ignore, so don't need to check for them.
		else if ( TitanDebug_GetSetting( "titan_debug_wave_no_block_despawn" ) && projectile.ProjectileGetWeaponInfoFileKeyField( "projectile_ignores_vortex" ) == "mirror" )
		{
			projectile.proj.hasBouncedOffVortex = true
			return false
		}
	...
	else
	{
		...
		if ( TitanDebug_GetSetting( "titan_debug_vortex_railgun_one_charge" ) && "bulletsToFire" in projectile.s )
		{
			impactData.bulletsToFire <- 1
			impactData.extraDamagePerBullet <- expect int( projectile.ProjectileGetWeaponInfoFileKeyField( "damage_additional_bullets" ) )
			impactData.extraDamagePerBullet_Titan <- expect int( projectile.ProjectileGetWeaponInfoFileKeyField( "damage_additional_bullets_titanarmor" ) )
		}
...
bool function Vortex_FireBackExplosiveRound( vortexWeapon, attackParams, impactData, sequenceID )
{
	...
	// fire off the bolt
	entity bolt = vortexWeapon.FireWeaponBolt( attackPos, fireVec, projSpeed, damageType, DF_IMPACT | damageType, PROJECTILE_NOT_PREDICTED, sequenceID )
	if ( bolt )
	{
		bolt.kv.gravity = 0.3
		if ( TitanDebug_GetSetting( "titan_debug_vortex_carry_effects" ) )
...
bool function DoVortexAttackForImpactData( entity vortexWeapon, attackParams, impactData, int attackSeedCount )
{
	...
	if ( TitanDebug_GetSetting( "titan_debug_vortex_amplifier_projectiles" ) )
...
function Vortex_ProjectileCommonSetup( entity projectile, impactData )
{
	...
	if ( TitanDebug_GetSetting( "titan_debug_vortex_carry_effects" ) )
	{
		...
		AddEntityDestroyedCallback( projectile, TitanDebug_FixVortexRefireExplosion )
	}

	if ( TitanDebug_GetSetting( "titan_debug_vortex_railgun_one_charge" ) && "bulletsToFire" in impactData )
	{
		projectile.s.bulletsToFire <- impactData.bulletsToFire
		projectile.s.extraDamagePerBullet <- impactData.extraDamagePerBullet
		projectile.s.extraDamagePerBullet_Titan <- impactData.extraDamagePerBullet_Titan
	}
...
void function OnDamageAmpProjectile( entity victim, var damageInfo )
{
    ...
    DamageInfo_ScaleDamage( damageInfo, 1.35 ) // 1.35 matches Vortex Amp value
}
...
void function TitanDebug_FixTetherReflect( entity projectile )
{
	if ( projectile.ProjectileGetWeaponClassName() != "mp_titanability_tether_trap" )
		return
	
	// Reapplies init values from tether file. Otherwise, they are invincible.
	projectile.proj.onlyAllowSmartPistolDamage = false
	SetCustomSmartAmmoTarget( projectile, true )
...
bool function CodeCallback_OnVortexHitProjectile( entity weapon, entity vortexSphere, entity attacker, entity projectile, vector contactPos )
{
	...
	int teamNum = vortexSphere.GetTeam()
		...
		if ( TitanDebug_GetSetting( "titan_debug_particle_projectile_damage" ) )
...
void function TitanDebug_FixVortexRefire( entity victim, var damageInfo )
{
...
function TitanDebug_FixVortexRefireExplosion( projectile )
{
	...
	float explosionDamageModTitan = 0.0

	// Need to go npc_normal -> normal
	for ( int i = start + 1; i < keys.len(); i += 2 )
	{
		explosionDamageMod = float( GetWeaponInfoFileKeyField_WithMods_Global( weaponName, mods, keys[start] ) )
		if ( explosionDamageMod > 0 )
			break
	}

	// Need to go npc_heavy_armor -> heavy_armor -> normal
	for ( int i = start; i < keys.len(); i += ( i > 1 ? 1 : 2 ) )
	{
		explosionDamageModTitan = float( GetWeaponInfoFileKeyField_WithMods_Global( weaponName, mods, keys[start] ) )
		if ( explosionDamageModTitan > 0 )
			break
	}
...
// Used to fix Vortex reflected projectile damage.
