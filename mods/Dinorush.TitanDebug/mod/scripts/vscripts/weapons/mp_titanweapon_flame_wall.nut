...
bool function CreateThermiteWallSegment( entity projectile, int projectileCount, entity inflictor, entity movingGeo, vector pos, vector angles, int waveCount )
{
	...
	if ( projectile.proj.savedOrigin != < -999999.0, -999999.0, -999999.0 > )
	{
		...
		if ( !movingGeo )
		{
			...
			AI_CreateDangerousArea_Static( thermiteParticle, projectile, METEOR_THERMITE_DAMAGE_RADIUS_DEF, TitanDebug_GetSetting( "titan_debug_ai_dodge_friendly" ) ? owner.GetTeam() : TEAM_INVALID, true, true, pos )
		}
		else
		{
			if ( GetMapName() == "sp_s2s" )
			...
			AI_CreateDangerousArea( thermiteParticle, projectile, METEOR_THERMITE_DAMAGE_RADIUS_DEF, TitanDebug_GetSetting( "titan_debug_ai_dodge_friendly" ) ? owner.GetTeam() : TEAM_INVALID, true, true )
		}
