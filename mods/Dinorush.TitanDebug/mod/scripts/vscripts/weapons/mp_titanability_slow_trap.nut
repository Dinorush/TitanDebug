...
// TODO: Fix canister double tick rate + multi ignition + inconsistent ticking.
//	Double tick rate: Maintain a counter + start time per entity. Don't do damage if it counter would exceed the value it should be. Reset start time if time diff > 0.25 or 0.3
//  Multi ignition: Ez
//  Inconsistent ticking: Use a buffer system local to function.
...
const float TITAN_DEBUG_SINGLE_TICK_DAMAGE_BONUS = 1.25
...
function IgniteTrap( entity damageArea, var damageInfo, bool isExplosiveBarrel = false )
{
	...
	// Expiration damage ignores Tempered Plating and does bonus damage because the inflictor dies early
	// which stops the damage callback from modifying damage dealt.
	float durationFix = TitanDebug_GetSetting( "titan_debug_canister_no_expiration_damage" ) ? 0.5 : 0.0
	entity inflictor = CreateOncePerTickDamageInflictorHelper( duration + durationFix )
	inflictor.SetOrigin( origin )

	if ( TitanDebug_GetSetting( "titan_debug_canister_no_double_tick" ) )
		inflictor.s.titanDebugSlowTrapEnts <- {}
	...
	if ( movingGeo )
	{
		...
		AI_CreateDangerousArea( inflictor, weapon, dangerousAreaRadius, TitanDebug_GetSetting( "titan_debug_ai_dodge_friendly" ) ? owner.GetTeam() : TEAM_INVALID, true, true )
	}
	else
	{
		AI_CreateDangerousArea_Static( inflictor, weapon, dangerousAreaRadius, TitanDebug_GetSetting( "titan_debug_ai_dodge_friendly" ) ? owner.GetTeam() : TEAM_INVALID, true, true, originNoHeightAdjust )
	}
...
void function FireTrap_DamageAreaOverTime( entity owner, entity inflictor, vector pos, float duration )
{
	...
	// Buffer system: Count the time that we wait minus 0.2s. Wait 0.2s - buffer.
	// This ensures we maintain an average of 0.2s tick rate over the trap lifetime
	// by lowering wait time if it waits for longer than it should.
	float tickBuffer = 0.0
	float endTime = Time() + duration
	while ( Time() < endTime )
	{
		FireTrap_RadiusDamage( pos, owner, inflictor )
		tickBuffer = TitanDebug_WaitBuffer( 0.2, tickBuffer, "titan_debug_thermite_tick_rate" )
	}
}
...
void function FireTrap_RadiusDamage( vector pos, entity owner, entity inflictor )
{
	...
	if ( TitanDebug_GetSetting( "titan_debug_canister_no_double_tick" ) )
		METEOR_DAMAGE_TICK *= TITAN_DEBUG_SINGLE_TICK_DAMAGE_BONUS
...
void function FireTrap_DamagedPlayerOrNPC( entity ent, var damageInfo )
{
	...
	if ( !TitanDebug_SlowTrapShouldDamage( ent, inflictor ) )
		return
...
bool function TitanDebug_SlowTrapShouldDamage( entity ent, entity inflictor )
{
	if ( !TitanDebug_GetSetting( "titan_debug_canister_no_double_tick" ) )
		return true

	if ( !( ent in inflictor.s.titanDebugSlowTrapEnts ) )
		inflictor.s.titanDebugSlowTrapEnts[ent] <- [0, Time()]
	
	array slowTrapInfo = expect array( inflictor.s.titanDebugSlowTrapEnts[ent] )
	// If enough time has passed since damage was last dealt, the player must have left the trap and re-entered.
	// Reset their info so double tick rate doesn't occur.
	if ( Time() - slowTrapInfo[1] >= 0.3 )
	{
		slowTrapInfo[0] = 0
		slowTrapInfo[1] = Time()
	}

	// Calculate the maximum number of times they should've been hit.
	// Let damage through if they have been hit fewer times than that.
	int tickCap = int( ( Time() - slowTrapInfo[1] ) / 0.2 ) + 1
	if ( slowTrapInfo[0] >= tickCap )
		return false
